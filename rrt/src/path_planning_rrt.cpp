...
public:
  ...
  bool checkObstacles(double &x1,
                      double &y1, double &x2,
                      double &y2,
                      std::tuple<std::vector<double>, std::vector<double>, std::vector<double>, std::vector<double>> obs)
  {
    bool decision;
    ...
    for (long unsigned i = 0; i < minX.size(); i++)
    {
      // Completely outside
  ...
  Node *checkNearestNode(Node *new_node)
  {
    ...
    if (minDistance <= MAX_DISTANCE)
    {
      corrX = near_node->posX;
      corrY = near_node->posY;
    }
    ...
    if (
        ((double)new_node->posX == (double)this->goal->posX) && ((double)new_node->posY == (double)this->goal->posY))
    {
      std::cout << "Goal achieved and Goal path is: " << std::endl;
