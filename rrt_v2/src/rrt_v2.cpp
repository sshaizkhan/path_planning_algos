 ...
 * @cite derived from: https://github.com/vss2sn/path_planning/blob/master/src/rrt.cpp
 */
...
// const
constexpr double half_grid_unit = 0.5;      // Add to json
constexpr double precison_limit = 0.000001; // ADD TO JSON
...
bool RRT::IsAnyObstacleInPath(const Node &n1, const Node &n2) const
{
  if (n2.y_ - n1.y_ == 0)
  {
    const double c = n2.y_;
    return std::any_of(
        std::begin(obstacles_list_), std::end(obstacles_list_),
        [&c, &n1, &n2](const Node &obs_node) {
          return obs_node.y_ == c &&
                 ((n1.x_ >= obs_node.x_ && obs_node.x_ >= n2.x_) ||
                  (n1.x_ <= obs_node.x_ && obs_node.x_ <= n2.x_));
        });
  }

  const double slope = static_cast<double>(n2.x_ - n1.x_) / (n2.y_ - n1.y_);
  const double c = n2.x_ - slope * n2.y_;

  for (const auto &obs_node : obstacles_list_)
  {
    if (!(((n1.y_ >= obs_node.y_) && (obs_node.y_ >= n2.y_)) ||
          ((n1.y_ <= obs_node.y_) && (obs_node.y_ <= n2.y_))))
    {
      continue;
    }

    if (!(((n1.x_ >= obs_node.x_) && (obs_node.x_ >= n2.x_)) ||
          ((n1.x_ <= obs_node.x_) && (obs_node.x_ <= n2.x_))))
    {
      continue;
    }
...
std::tuple<bool, std::vector<Node>> RRT::Plan(const Node &start, const Node &goal)
{
  start_ = start;
  goal_ = goal;
  ...
  while (iteration < max_iterations)
  {
    ...
    // Go back to beginning of the loop if point is already cosidered
    if (grid_[new_node.x_][new_node.y_] != 0)
    {
      continue;
    }

    //  Go back to beginning of loop if no near neighbor
...
{
  auto dist = std::sqrt(
      std::pow(goal_.x_ - new_node.x_, 2) +
...
int main()
{
  ...
  // Make sure start and goal are not obstacles and thier ids are not conflicting
  ...
  std::cout << "Found Path: " << found_path << std::endl;
